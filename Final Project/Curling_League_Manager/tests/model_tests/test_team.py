...
class TeamTests(unittest.TestCase):
    ...
    def test_adding_member_with_duplicate_oid(self):
        t = Team(1, "Flintstones")
        tm1 = TeamMember(5, "f", "f")
        tm2 = TeamMember(5, "g", "g")
        t.add_member(tm1)
        self.assertIn(tm1, t.members)
        self.assertRaises(DuplicateOid, t.add_member, tm2)

        # make sure tm2 is not in team.
        # cant run this test because the __eq__ checks if the oid's are the same.
        # self.assertNotIn(tm2, t.members)

    def test_adding_member_with_duplicate_email(self):
        t = Team(1, "Flintstones")
        tm1 = TeamMember(5, "f", "same")
        tm2 = TeamMember(6, "g", "same")
        t.add_member(tm1)
        self.assertIn(tm1, t.members)
        self.assertRaises(DuplicateEmail, t.add_member, tm2)
    ...
    def test_sends_real_email(self):
        t = Team(1, "Flintstones")
        tm1 = TeamMember(5, "f", "ds1.testing.001@gmail.com")
        tm2 = TeamMember(6, "g", "dsample@knology.net")
        t.add_member(tm1)
        t.add_member(tm2)
        re = Emailer()
        t.send_email(re, "S", "M")
